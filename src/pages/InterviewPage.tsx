...
export default function InterviewPage() {
  ...
  const aiModels = [
    ...
    { value: 'anthropic/claude-3-sonnet', label: 'Claude 3 Sonnet', description: 'Excellent for behavioral questions' },
    { value: 'openai/gpt-4-turbo', label: 'GPT-4 Turbo', description: 'Well-rounded for all interview types' },
  ...
  const processAudioRecording = async (audioBlob: Blob) => {
    try {
      ...
      toast({
        ...
        description: error.message || "Could not process audio. Please try again or type your answer.",
        variant: "destructive",
      });
    } finally {
  ...
  const renderMessage = (message: any) => (
    <div key={message.id} className={`flex gap-3 ${message.role === 'candidate' ? 'flex-row-reverse' : ''}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
        message.role === 'interviewer' ? 'bg-gradient-to-br from-blue-500 to-purple-600 text-white' : 'bg-gradient-to-br from-green-500 to-teal-600 text-white'
      }`}>
                      ...
                      {difficulties.map(diff => (
                        <Card 
                          ...
                          className={`cursor-pointer transition-all hover:shadow-md ${
                            selectedDifficulty === diff.value ? 'ring-2 ring-primary shadow-md' : ''
                          }`}
