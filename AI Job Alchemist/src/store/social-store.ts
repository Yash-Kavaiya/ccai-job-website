  ...
  persist(
    (set, get) => ({
      ...
      loadConnections: async () => {
        set({ isLoading: true, error: null });
        try {
          const response = await table.getItems('ewrtoiv0vw1s');
          const acceptedConnections = response.items.filter((c: any) => c.connection_status === 'accepted');
      ...
      loadConnectionRequests: async () => {
        ...
        try {
          const response = await table.getItems('ewrtoiv0vw1s');
          const pendingRequests = response.items.filter((c: any) => c.connection_status === 'pending');
